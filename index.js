...
async function initDatabase() {
    try {
        ...
        const bridgeResult = await db.query(
            "SELECT guild_id, channel_id FROM bridge_channels"
        );
        ...
        const blacklistResult = await db.query(
            "SELECT guild_id FROM blacklisted_guilds"
        );
        ...
        console.log(
            `Loaded ${bridgeResult.rows.length} bridge channel(s) from database`
        );
        console.log(
            `Loaded ${blacklistResult.rows.length} blacklisted guild(s) from database`
        );
    } catch (error) {
...
async function blacklistGuild(
    ...
    try {
        ...
        console.log(
            `Blacklisted guild ${guildName} (${guildId}) by ${blacklistedBy}`
        );
    } catch (error) {
...
async function unblacklistGuild(guildId) {
    try {
        await db.query("DELETE FROM blacklisted_guilds WHERE guild_id = $1", [
            guildId,
        ]);
...
async function stripEmojis(msg) {
    ...
    for (const emoji of emojis) {
        const response = await fetch("https://cdn.fl1nt.dev/api/urls", {
            method: "POST",
            body: JSON.stringify({
                url: `https://cdn.discordapp.com/emojis/${emoji
                    .split(":")[2]
                    .replace(">", "")}.${
                    emoji.includes("<a") ? "gif" : "png"
                }?v=1`,
            }),
            headers: {
        ...
        msg = msg.replace(emoji, response.data.shortCode);
    }
    return msg;
}
...
discordClient.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.commandName === "setchannel") {
        ...
        if (blacklistedGuilds.has(interaction.guildId)) {
            await interaction.reply({
                content: "this server is blacklisted from colonthree.",
                ephemeral: true,
            });
            return;
        }
    ...
    if (interaction.commandName === "blacklist") {
        ...
        if (!allowedUsers.includes(interaction.user.id)) {
            await interaction.reply({
                content:
                    "who do you think you are?? you can't use this command.",
                ephemeral: true,
            });
            return;
        }
...
discordClient.on("messageReactionAdd", async (reaction, reactor) => {
    if (
        !bridgeChannels.has(reaction.message.guildId) ||
        bridgeChannels.get(reaction.message.guildId) !==
            reaction.message.channelId ||
        blacklistedGuilds.has(reaction.message.guildId)
    )
        return;
    const reactorMember = await reaction.message.guild.members.fetch(
        reactor.id
    );
            ...
            message: `reacted to ${
                ...
                message.content
                    ? message.content
                          ...
                          .slice(0, 5)
                          .join(" ") +
                          (message.content.split(" ").filter((x) => !!x)
                              .length > 5
                              ? "..."
                              : "")
                    : "Multimedia message"
            ...
            msg.content = `Reply to ${
                ...
                repliedMessage.content
                    ? repliedMessage.content
                          ...
                          .join(" ")
                          .slice(0, 50) +
                          (repliedMessage.content.split(" ").filter((x) => !!x)
                              .length > 5 || repliedMessage.content.length > 50
                              ? "..."
                              : "")
                    : "Multimedia message"
            )}): ${msg.content}`;
